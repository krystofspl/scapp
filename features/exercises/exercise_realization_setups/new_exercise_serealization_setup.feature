Feature: Add new exercise realization setup
  In order to manage exercise realizations
  As admin or coach
  I want to add new exercise realization setup
  ...
  @javascript
  Scenario: As a coach, who created the exercise realization, I can add setups to it
    ...
    And I click Add for exercise setup "setup1"
    Then I should see an exercise realization setup "setup1"
    When I fill in exercise realization setup fields
      | value | note       |
      | asd   | setup note |
    And I click "Save"
    Then I should see an error message near the form
    When I fill in exercise realization setup fields
      | value | note       |
      | 5     | setup note |
    And I click "Save"
    Then I should see an exercise realization setup "setup1" with text "Value: 5.0 kg"

  @javascript
  Scenario: Required Exercise Setups should automatically be added when the realization is created
    Given I have "coach" role
    And I am at the "/scheduled_lessons/training-5-5-2050-13-00-14-00/exercise_realizations" page
    When I click Edit for exercise realization "excprivate" in plan for user "test2"
    And I click "Edit setups" tab
    Then I should see an exercise realization setup "setup2" with text "Value: 0.0 kg"
